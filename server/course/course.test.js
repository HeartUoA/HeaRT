...
describe('/api/course tests', () => {
  ...
  it('should post and get courses', async () => {
    ...
    const postResponse = await request(app)
      .post('/api/course')
      .send({ name: 'SOFTENG 700', date: dateNow, cohortSize: 200, role: 'Course Cordinator', ageOfCourse: 5 });
    expect(postResponse.status).toBe(200);
    expect(postResponse.body).toBe('Course saved!');

    const getResponse = await request(app).get('/api/course');
    expect(getResponse.status).toBe(200);
    expect(getResponse.body).toEqual([expect.objectContaining({ name: 'SOFTENG 700', cohortSize: 200, role: 'Course Cordinator', ageOfCourse: 5 })]);
